...
Provides a read-only data source to retrieve information about an iam_account_settings_template. You can then reference the fields of the data source in other resources within the same configuration by using interpolation syntax.

## Example Usage

```hcl
data "ibm_iam_account_settings_template" "iam_account_settings_template" {
	template_id = "template_id"
	version = ibm_iam_account_settings_template.iam_account_settings_template_instance.version
}
```
...
* `account_settings` - (List) Input body parameters for the Account Settings REST request.
